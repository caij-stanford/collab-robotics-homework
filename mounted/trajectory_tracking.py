...
class TrajectoryTracker(Node):
    ...
    def odom_callback(self, msg: Odometry):
        ...
        self.current_theta = 2 * np.arctan2(qz, qw) - np.pi/2  # Adjust for MuJoCo offset
    ...
    def control_loop(self):
        ...
        if not self.odom_received or not self.running:
            # self.get_logger().info(f'Waiting for odometry...')
            return
        
        # Uses system time if sim time is not available
        ...
        # Convert to robot commands (v, omega)
        ...
        # self.get_logger().info(f'Time: {t:.2f}s, Pos: ({self.current_x:.2f}, {self.current_y:.2f}), Error: ({error_x:.2f}, {error_y:.2f})')
